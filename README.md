...
> AI 驱动的全平台收藏与工具管理套件
>
> AI-powered, cross-platform bookmark & tool management suite

一站式管理你的 **GitHub Stars**、**网站书签**、**实用工具**，支持 AI 自动标签、语义搜索、Gist 多端同步、浏览器扩展、移动端与微信小程序。
...
| ☁️ 云端同步 | 通过 GitHub Gist 在多台设备间同步数据库 |
...
打开 Web 应用后，左侧为玻璃拟态风格导航栏，右侧为主内容区：

- 📊 **仪表盘**：数据概览、快捷操作、最近添加、收藏增长趋势
- 🔍 **AI 搜索**：用自然语言语义搜索你的收藏
- ⭐ **仓库列表**：同步的 GitHub Starred Repositories
- 🌐 **网站列表**：手动添加或从浏览器扩展导入的网站书签
- 📦 **软件列表**：收藏的软件/应用
- 🛠️ **工具列表**：收藏的实用工具/CLI
- 🕸️ **标签网络**：可视化探索标签之间的共现关系
- 📈 **统计面板**：类型/语言/标签分布、Stars 分布、增长趋势
- 🧰 **工具箱**：JSON、Base64、URL、时间戳、正则、颜色、Markdown、二维码
- 🔄 **导入导出**：JSON 备份恢复、HTML 书签导入
- ⚙️ **设置**：管理 Token、AI Key、Gist 同步、主题切换
- 👤 **个人主页**：个人统计、高星仓库 Top 10、功能状态

### 🔗 同步 GitHub Stars

1. 点击左侧导航 **设置**
2. 在 **GitHub** 卡片中找到 **GitHub Personal Access Token** 输入框
3. 填入你的 Token
   - 访问 https://github.com/settings/tokens
   - 点击 **Generate new token (classic)**
   - 勾选 `public_repo` 或 `repo`（访问私有仓库需要）以及 `read:user`
   - 生成后复制 Token
4. 切换到 **仪表盘**，点击 **同步 Stars** 按钮
5. 等待同步完成，系统会显示新增项目数量
...
1. 进入 **设置** 页面
2. 在 **AI 服务** 卡片中填入你的 **OpenAI API Key**
3. Key 仅保存在浏览器本地，不会上传到任何服务器

#### AI 标签生成

- **批量生成**：在 **仪表盘** 点击 **AI 标签**，系统会为没有标签的项目自动推荐标签
...
1. 在 **仪表盘** 点击 **生成 Embedding**
2. 系统会为每个项目生成向量表示
3. 生成后语义搜索更准确，也可查看 **相似推荐**
...
1. 进入 **设置** 页面
2. 在 **云端同步** 卡片的 **Gist ID** 输入框中：
   - **留空**：首次同步会自动创建一个新的 Secret Gist
   - **填入已有 Gist ID**：会同步到该 Gist
3. 点击 **同步到 Gist**
4. 同步成功后，Gist ID 会自动保存到 localStorage
5. 在另一台设备上输入相同的 Gist ID 并同步，即可拉取数据
...
Made with ❤️ by [TaotaoByte](https://github.com/TaotaoByte)
